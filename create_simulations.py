...
nTrials_per_stim = 500    # numero di prove per simulazione
...
# Crea la cartella principale "Simulations" se non esiste già
...
for simIdx in range(simReps):
    ...
    # Crea la sottocartella per il soggetto (subjectxx)
    subject_dir = os.path.join(base_dir, f"subject{simIdx:02d}")
    ...
    for sigIdx in range(len(w_sig)):
        for noiseIdx in range(len(w_noise)):
            # Calcola l'indice univoco per i pesi (00-99)
...
#            Y_tdel = [",".join(map(str, Y[t_del, i])) for i in range(nTrials)]
            ...
            # Crea il DataFrame in cui ogni riga rappresenta un trial
            df = pd.DataFrame({
                'S': S,
                'x_noise_t': x_noise_t,
                'x_signal_t': x_signal_t,
                'Y_t': Y_t,
                'Y_tdel': Y_tdel
            })
            
            # Definisci il nome del file: weightyy.csv (all'interno della sottocartella subjectxx)
            filename = f"weight{weight_index:02d}.csv"
            filepath = os.path.join(subject_dir, filename)
            # Salva il file CSV usando il punto e virgola come separatore di colonne
